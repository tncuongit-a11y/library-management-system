...
const seedData = async () => {
    try {
        ...
        console.log('🌱 Seeding initial library management data...');
        ...
        await Borrow.insertMany([
            { docGia: readers[0]._id, sach: books[0]._id, ngayMuon: new Date(Date.now() - 5 * 86400000), ngayTra: null },
            { docGia: readers[1]._id, sach: books[3]._id, ngayMuon: new Date(Date.now() - 10 * 86400000), ngayTra: new Date() },
            { docGia: readers[2]._id, sach: books[4]._id, ngayMuon: new Date(Date.now() - 2 * 86400000), ngayTra: null }
        ]);
